...
from dataloaders.dataset_semi_mscmr_v5 import (BaseDataSets, RandomGenerator,TwoStreamBatchSampler)
...
parser.add_argument('--root_path', type=str,
                    default='/mnt/sdd/tb/data/MSCMR', help='Name of Experiment')
parser.add_argument('--exp', type=str,
                    default='MSCMR_Semi/Mean_Teacher', help='experiment_name')
...
parser.add_argument("--kd_weights", type=int, default=0.8)
...
def train(args, snapshot_path):
    ...
    db_train_labeled = BaseDataSets(base_dir=args.root_path, num=8, labeled_type="labeled", split="train", transform=transforms.Compose([
        RandomGenerator(args.patch_size)]),label_ratio=args.label_ratio)
    ...
    for epoch_num in iterator:
        ...
        for i, data in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            #TODO: pCE loss
            loss_ce = ce_loss(seg[:args.labeled_bs,...], label_batch[:].long())
            ...
            crop_out=F.softmax(crop_out, dim=1) 
...
def backup_code(base_dir):
    ...
    dataset_name = 'dataset_semi_mscmr.py'
