    ...
    for epoch_num in iterator:
        ...
        #         # -2: padded pixels;  -1: unlabeled pixels (其中60%-70%是没有标注信息的)
         ...
         #aff_loss
            ...
            affinityenergyloss,pseudo_label = affinityenergyLoss(outs, att, unlabeled_RoIs,label_batch)
            pseudo_label = torch.argmax(pseudo_label.detach(), dim=1, keepdim=False)
            affinity_loss = losses.get_aff_loss(attpred[:args.labeled_bs,...],pseudo_label)

            loss_ce_wr = ce_loss(outputs[:args.labeled_bs,...], pseudo_label[:].long())
            ...
            loss = 8*supervised_loss+affinityenergyloss*args.kd_weights+affinity_loss+consistency_weight*consistency_loss 
