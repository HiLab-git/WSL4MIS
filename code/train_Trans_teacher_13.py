...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i, data in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            outputs,attpred,att,out_feats,class_seg= model(volume_batch) 
        ...
        #         # -2: padded pixels;  -1: unlabeled pixels (其中60%-70%是没有标注信息的)
         ...
         #aff_loss
            ...
            create_center_1_bg = class_seg[:,0,...].unsqueeze(1)# 4,1,x,y,z->4,2
            create_center_1_a =  class_seg[:,1,...].unsqueeze(1)
            create_center_1_b =  class_seg[:,2,...].unsqueeze(1)
            create_center_1_c =  class_seg[:,3,...].unsqueeze(1)



            create_center_2_bg = calss_ema[:,0,...].unsqueeze(1)
            create_center_2_a =  calss_ema[:,1,...].unsqueeze(1)
            create_center_2_b =  calss_ema[:,2,...].unsqueeze(1)
            create_center_2_c =  calss_ema[:,3,...].unsqueeze(1)
            ...
            loss = 5*supervised_loss+affinityenergyloss*args.kd_weights+affinity_loss+consistency_weight*consistency_loss+loss_contrast 
