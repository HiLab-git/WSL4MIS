...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i, data in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            outputs,attpred,att =model(volume_batch) 
            ...
            outputs_pred=F.interpolate(outputs[0], size=volume_batch.shape[2:], mode='bilinear', align_corners=True)

            #TODO: pCE loss
            loss_ce = ce_loss(outputs_pred[:args.labeled_bs,...], label_batch[:].long())
            ...
            # att1=att[3][1][:args.labeled_bs,...]
            # att2=att2[3][1][:args.labeled_bs,...]
            # att1 = F.interpolate(att1, scale_factor=scale_factor, mode='bilinear', align_corners=True)            
            # att2 = F.interpolate(att2, size=img2.shape[2:], mode='bilinear', align_corners=True)


            # loss_er = torch.mean(torch.abs(att1 - att2))
        ...
        #         # -2: padded pixels;  -1: unlabeled pixels (其中60%-70%是没有标注信息的)
         ...
         #aff_loss
            # outs = []        
            # outs.append(outputs[0])  #[:args.labeled_bs,...]          
            # outs.append(outputs[1])
            # outs.append(outputs[2])
            # outs.append(outputs[3])
            ...
            unlabel_loss = ce_loss(outputs_pred[args.labeled_bs:,...],
                                    pseudo_label[args.labeled_bs:,...][:].long())+dice_loss(outputs_pred[args.labeled_bs:,...], pseudo_label[args.labeled_bs:,...].unsqueeze(1))
            ...
            loss = 5*supervised_loss+affinity_loss+local_affinity_loss*args.kd_weights+consistency_weight*unlabel_loss
...
def backup_code(base_dir):
    ...
    shutil.copy(val_name, code_path + '/' + val_name)
