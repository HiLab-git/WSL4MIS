...
import math
from utils.util import cams_to_refine_label
...
def train(args, snapshot_path):
    ...
    aff_2_pseudo_label=losses.SegformerAffinityEnergyLoss()
    ...
    for epoch_num in iterator:
        ...
        for i, data in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            outputs,out_feats,attpred,att =model(volume_batch) 

            outputs_unlabeled_soft = torch.softmax(outputs[0][args.labeled_bs:,...], dim=1)
            outputs_seg_soft = torch.softmax(outputs[0][:args.labeled_bs,...], dim=1)

            outputs_unlabeled_soft_mlp = torch.softmax(out_feats[0][args.labeled_bs:,...], dim=1)
            outputs_seg_soft_mlp = torch.softmax(out_feats[0][:args.labeled_bs,...], dim=1)

            #TODO: pCE loss
            loss_ce = ce_loss(outputs[0][:args.labeled_bs,...], label_batch[:].long())


            #TODO: Equivariant Regularization Loss
            # scale_factor=0.3
            # img2 = F.interpolate(volume_batch[:args.labeled_bs,...], scale_factor=scale_factor, mode='bilinear', align_corners=True)
            # att2=model(img2,aux=True) 
            # # outputs_seg_soft_ER = torch.softmax(outputs_ER, dim=1)
            # att1 = F.interpolate(att[3][1], scale_factor=scale_factor, mode='bilinear', align_corners=True)
            # att2 = F.interpolate(att2, size=img2.shape[2:], mode='bilinear', align_corners=True)
            # loss_er = torch.mean(torch.abs(att1[:args.labeled_bs,...] - att2))


            with torch.no_grad():
                ema_output,_,_,_ = ema_model(ema_inputs)
                ema_output_soft = torch.softmax(ema_output[0][args.labeled_bs:,...], dim=1)
            ...
            #consistency loss
            # outputs_unlabeled_soft1 = torch.softmax(outputs[1][args.labeled_bs:,...], dim=1)
            # outputs_unlabeled_soft2 = torch.softmax(outputs[2][args.labeled_bs:,...], dim=1)

            # outputs_unlabeled_soft_mlp1 = torch.softmax(out_feats[2][args.labeled_bs:,...], dim=1)
            # outputs_unlabeled_soft_mlp2 = torch.softmax(out_feats[3][args.labeled_bs:,...], dim=1)

            # consistency_loss  = torch.mean((outputs_unlabeled_soft - outputs_unlabeled_soft_mlp) ** 2)
            # consistency_loss1  = torch.mean((outputs_unlabeled_soft1 - outputs_unlabeled_soft_mlp1) ** 2)
            # consistency_loss2  = torch.mean((outputs_unlabeled_soft2 - outputs_unlabeled_soft_mlp2) ** 2)
            # consistency_loss=consistency_loss#+consistency_loss1+consistency_loss2
        ...
        #         # -2: padded pixels;  -1: unlabeled pixels (其中60%-70%是没有标注信息的)
         ...
         #aff_loss
            ...
            local_affinity_loss,pseudo_label = aff_2_pseudo_label(outs, att, unlabeled_RoIs,label_batch)
            
            pseudo_label = torch.argmax(pseudo_label.detach(), dim=1, keepdim=False)
            ref_label = cams_to_refine_label(pseudo_label,  ignore_index=4)

            affinity_loss = losses.get_aff_loss(attpred[:args.labeled_bs,...],ref_label)

            loss_ce_wr = ce_loss(outputs[0][:args.labeled_bs,...], pseudo_label[:].long())

            loss_dice_wr= dice_loss(outputs_seg_soft, pseudo_label.unsqueeze(1))
            ...
            loss = 5*supervised_loss+affinity_loss+local_affinity_loss*args.kd_weights+consistency_weight*consistency_loss# +loss_er
...
def backup_code(base_dir):
    ...
    shutil.copy('utils/' + loss_name, code_path + '/' + loss_name)
    shutil.copy('utils/' + util_name, code_path + '/' + util_name)
