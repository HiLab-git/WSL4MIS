...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i, data in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            outputs_unlabeled_soft = torch.softmax(outputs_pred[args.labeled_bs:,...], dim=1)
            outputs_seg_soft = torch.softmax(outputs_pred[:args.labeled_bs,...], dim=1) 
            ...
            with torch.no_grad():
                outputs_ema,attpred_ema,att_ema = ema_model(ema_inputs)
                outputs_pred_ema=F.interpolate(outputs_ema[0], size=volume_batch.shape[2:], mode='bilinear', align_corners=True)
                ema_outputs_unlabeled_soft = torch.softmax(outputs_pred_ema[args.labeled_bs:,...], dim=1)
                ema_outputs_seg_soft = torch.softmax(outputs_pred_ema[:args.labeled_bs,...], dim=1) 


            #TODO: consistency loss
        ...
        #         # -2: padded pixels;  -1: unlabeled pixels (其中60%-70%是没有标注信息的)
            ...
            _,pseudo_label_ema = aff_2_pseudo_label(outputs_ema, att_ema, unlabeled_RoIs,label_batch)
            pseudo_label_ema = torch.argmax(pseudo_label_ema.detach(), dim=1, keepdim=False)

            loss_ce_wr = ce_loss(outputs_pred[:args.labeled_bs,...], pseudo_label_ema[:args.labeled_bs,...][:].long())

            loss_dice_wr= dice_loss(outputs_seg_soft, pseudo_label_ema[:args.labeled_bs,...].unsqueeze(1))

            unsup_loss = ce_loss(outputs_pred[args.labeled_bs:,...],
                                 pseudo_label_ema[args.labeled_bs:,...][:].long())+dice_loss(outputs_seg_soft, pseudo_label_ema[args.labeled_bs:,...].unsqueeze(1))
            
            supervised_loss=loss_ce+0.5*(loss_ce_wr+loss_dice_wr)


            loss = 5*supervised_loss+consistency_loss*consistency_weight+affinity_loss+local_affinity_loss*args.kd_weights+unsup_loss
