...
add attation and calssifiation
"""
...
from networks.head import SegFormerHead,class_Head
from networks.attention import Self_Attention as Attention
...
class Encoder_self_attention(nn.Module):
    def __init__(self, params):
        super(Encoder_self_attention, self).__init__()
        self.params = params
        self.in_chns = self.params['in_chns']
        self.ft_chns = self.params['feature_chns']
        self.n_class = self.params['class_num']
    ...
    def forward(self, x):
        ...
        x3 = self.down3(x2)
        x4 = self.down4(x3)
        att=x4.view(x4.shape[0],-1,x4.shape[1])
        att=self.attn(att,256,256)
        return [x0, x1, x2, x3, x4],att
...
class UNet_new(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder_self_attention(params)
        self.decoder = Decoder(params)

        self.seg_head=class_Head(
                            # type='SegFormerHead',
