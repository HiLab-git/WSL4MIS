...
def train(args, snapshot_path):
    ...
    #步骤四：定义数据集
    # train_datasets = ...#自己定义的Dataset子类
    # train_sampler_labeled = DistributedSampler(db_train_labeled)
    # train_sampler_unlabeled = DistributedSampler(db_train_unlabeled)

    # trainloader_labeled = DataLoader(db_train_labeled, sampler=train_sampler_labeled, batch_size=args.train_batch_size,
                                #   num_workers=args.num_workers, drop_last=True,pin_memory=True)
    # trainloader_unlabeled = DataLoader(db_train_unlabeled, sampler=train_sampler_unlabeled, batch_size=args.train_batch_size,
                                #   num_workers=args.num_workers, drop_last=True,pin_memory=True)
    ...
    # optimizer2 = optim.SGD(ema_model.parameters(), lr=base_lr,
    #                        momentum=0.9, weight_decay=0.0001)
    ...
    for epoch_num in iterator:
        ...
        for i, data in enumerate(zip(cycle(trainloader_labeled), trainloader_unlabeled)):
            ...
            ema_output,ema_calss,ema_attpred  = model2(ema_inputs)
            ...
            supervised_loss=loss_ce+loss_dice_wr+loss_ce_wr#loss_ce+loss_dice_wr+loss_ce_wr+loss_ce2+loss_dice_wr2+loss_ce_wr2
            ...
            create_center_2_bg = ema_calss.unsqueeze(1)
            create_center_2_a =  ema_calss.unsqueeze(1)
            create_center_2_b =  ema_calss.unsqueeze(1)
            create_center_2_c =  ema_calss.unsqueeze(1)
